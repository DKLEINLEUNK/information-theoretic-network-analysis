"""Sample size sweep: AIS bias vs N at fixed (phi, r).
...
N_GRID = np.logspace(1, 4, 10, dtype=int)
REPS_DEFAULT = 100
SEED_DEFAULT = 42

DEFAULT_OUTPUT_DIR = Path("./results/univariate/samples_sweep")
    ...
    for gen_name, gen in tqdm(generators.items(), desc="generators"):
        for n in n_grid:
            results = (
                ...
                .run(n=int(n), seed=seed, reps=reps)
            )
            for r in results:
                rows.append({
                    "generator": gen_name,
